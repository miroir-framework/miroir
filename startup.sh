cd ~/Mes\ documents/devhome/miroir-app/ && lerna run build --scope='miroir-datastore-postgres' && npx lerna run test --scope=miroir-standalone-app -- "DomainController.Model.CRUD"
cd ~/Mes\ documents/devhome/miroir-app/ && lerna run build --scope='miroir-redux' && lerna run watch --scope='miroir-redux'
cd ~/Mes\ documents/devhome/miroir-app/ && lerna run build --scope='miroir-datastore-postgres' && lerna run dev2 --scope='miroir-server'
