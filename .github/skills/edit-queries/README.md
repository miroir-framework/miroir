...
The `filter` attribute in `extractorTemplateForObjectListByEntity` and `extractorInstancesByEntity` now supports filtering on multiple values using the `values` array:
