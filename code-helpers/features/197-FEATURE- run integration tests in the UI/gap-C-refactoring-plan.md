...
(unchanged). Optional PersistenceStoreController→`domainController` migration in tests is
...
| **ExtractorTemplate** | `4_storage/ExtractorTemplatePersistenceStoreRunner.integ.test.tsx` | Setup → `AppStackIntegrationTestSession` (Slice ET ✅). Assertions: `handleQueryTemplateActionForServerONLY` on PersistenceStoreController (unchanged) |
| **PersistenceStoreController CRUD** | `4_storage/PersistenceStoreController.integ.test.tsx` | Setup → `AppStackIntegrationTestSession` (Slice P ✅). Assertions: `localAppPersistenceStoreController` / `localMiroirPersistenceStoreController` (unchanged) |
    ...
    runMiroirTests._runMiroirTestSuite(executionOptions: { integrationStore: sqlDbDataStore })
      runMiroirTest("transformerTest", integration)
        runMiroirTransformerIntegrationTest(sqlDbDataStore)
          sqlDbDataStore.handleBoxedQueryAction(runBoxedQueryAction)   ← PersistenceStoreController direct
```
...
**Goal (revised):** unify **setup** through `AppStackIntegrationTestSession`; **test bodies stay on PersistenceStoreController**
...
new AppStackIntegrationTestSession(miroirConfig, {
  ...
  → {
      domainController,                        // client domainController (remote PersistenceStoreController mode)
      applicationDeploymentMap,
...
**After (test bodies)** — unchanged (still PersistenceStoreController direct calls)
...
**test bodies stay on PersistenceStoreController** (`handleQueryTemplateActionForServerONLY`). No migration of assertion
...
### 2.4 PersistenceStoreController CRUD tests — Slice P (`PersistenceStoreController.integ.test.tsx`) — ✅ done
...
**Non-goal (out of scope for Slice P):** replacing PersistenceStoreController calls in tests with `domainController` — see
...
`initMiroirCoreTestIntegrationStore` direct-PersistenceStoreController assembly.
...
export class IntegrationTestSession implements RunnerTestSessionInterface {
  ...
  async initSession(): Promise<MiroirTestExecutionEnvironment> {
    ...
    const domainController = await setupMiroirDomainController(miroirContext, { local PersistenceStoreController });
    // PersistenceStoreController createStore / open / initApplication for admin + test deployments
...
tests can keep PersistenceStoreController-direct assertions.
...
**Role:** Closes the last direct PersistenceStoreController call inside `miroir-core/src`. After this change,
...
| A new `PersistenceStoreController` unit test file | The CRUD behaviour is already covered by `DomainController.integ.Data.CRUD.test.tsx`; a separate unit test for PersistenceStoreController is a separate future concern, not part of this migration |
...
| `setupMiroirTest` (miroir-react) | `setupMiroirDomainController` (miroir-localcache-redux) — direct PersistenceStoreController + domainController, no emulated HTTP |
...
- Assert `env.persistenceStoreControllerManager.getPersistenceStoreController(INTEG_TEST_DEPLOYMENT_UUID)` returns the opened PersistenceStoreController used for seeding.
...
### Slice P — PersistenceStoreController CRUD tests (`PersistenceStoreController.integ.test.tsx`) — ✅ done
...
## Appendix A — deferred PersistenceStoreController assertion migration
...
from direct PersistenceStoreController calls to `domainController`:

| PersistenceStoreController call | `domainController` equivalent |
|----------|-------------------------------|
...
Introduce `buildGetInstancesQuery` as a **private** test-file helper only. Remove PersistenceStoreController variables
...
- [x] `ExtractorPersistenceStoreRunner.integ.test.tsx` uses `AppStackIntegrationTestSession` in `beforeAll`; PersistenceStoreController variables wired from manager
- [x] Extractor `it()` bodies still call `handleBoxedQueryAction` on PersistenceStoreController (unchanged)
...
- [x] `ExtractorTemplatePersistenceStoreRunner.integ.test.tsx` uses `AppStackIntegrationTestSession` in `beforeAll`; PersistenceStoreController variables wired from manager
- [x] `localCache` obtained from `domainController.getLocalCache()` after session init (for unchanged `beforeEach`)
- [x] ExtractorTemplate `it()` bodies still call `handleQueryTemplateActionForServerONLY` on PersistenceStoreController (unchanged)
...
### Slice P (PersistenceStoreController CRUD setup via `AppStackIntegrationTestSession`) — done
