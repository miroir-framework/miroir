...
- **`initMiroirCoreTestIntegrationStore`** should be replaced by a call to
  `setupMiroirTest` (or the common orchestrator's `PostgresIntegrationAdapter`) that returns a
...
- The `RunnerTestSessionInterface` (already defined in `miroir-core`) is the right seam, but
  only `RunnerTestSession` (standalone-app) currently implements it. `PostgresIntegrationAdapter`
  (core) is planned but not wired to a common `MiroirTestIntegrationOrchestrator`.

### What needs to be filled

- Implement `PostgresIntegrationAdapter` in `miroir-core` as the adapter for transformer integ
...
- Wire both adapters (`PostgresIntegrationAdapter` for core, `RunnerTestSession` for standalone)
...
| **E** — Setup helper fragmentation | `PostgresIntegrationAdapter`, wired `MiroirTestIntegrationOrchestrator`, deprecation of ad-hoc `setupMiroirTest*` ladder | All families | Yes (enables Phase B) |
