...
**Status:** Slices 7, 8, 9, 10, and 11 ✅ DONE.
...
The new entry's verbose list is diffed against `baseline-JzodElementEditor.txt` for the suite. The development-build check from §4.4 repeats for the suite. Slice 11 leaves the old file with no active suite. vitest fails a file with no test, so the old file keeps one placeholder test until Slice 12 (see the Slice 11 realization).
...
#### Slice 9 realization (JzodSimpleTypeEditor)

**RED observed.** The 12 SimpleType cases were added to `componentTestManifest.ts` (`JzodSimpleTypeEditor`, after `JzodObjectEditor`) and to `componentTestRegistry.ts` through a new `jzodElementEditor/JzodSimpleTypeEditor.tsx` whose 12 bodies threw "not migrated". The generator printed "5 suite(s), 44 case(s)" and wrote the instance once, then "no change" on a second run. `npm run build -w miroir-test-app_deployment-miroir` succeeded. The new entry, run with `npx vitest run` without bail (`VITE_TEST_MODE=true`): 12 failed, 33 passed (45). Each failure reads `reactComponentTest "JzodElementEditor_ComponentTestSuite#JzodSimpleTypeEditor#JzodSimpleTypeEditor: <case>" failed: not migrated`. The 32 Array, Enum, Literal, and Object cases and the entry check passed.

**GREEN.**

- `jzodElementEditor/JzodSimpleTypeEditor.tsx`: the 12 cases of `getJzodSimpleTypeEditorTests` (string, number, uuid, boolean, and bigint; the commented-out "bigint … as number" case was not ported), with the §5.3 rules (`screen` to `env.view`, `expect` to `env.expect`, `container` to `env.container` as root of every `extractValuesFromRenderedElements` call, React `act` to `env.act`, `fireEvent` to `env.fireEvent`, and `waitAfterUserInteraction()` to the act-free `waitAfterUserInteraction(env.container)`). The one active `screen.debug` call (in "bigint renders input…") is deleted. No case opens a dropdown or reads portaled options, so no call passes `env.portalElement`, and no case uses `userEvent` or `log`. As in Slice 8, the old suite had no suite props, so each case keeps its full props, with the shared label, name, and list keys in a local `testFieldProps`. `suiteProps` (required by `ComponentTestSuite`) repeats the first case's props and no case uses it. Three local helpers hold code repeated by the old bodies: `testFieldTextBox(env)` (the textbox named `TESTSECTION.testField`), `numberInput42(env)` (the input with id `testField` displaying 42), and `testFieldValues(env, step)` (`extractValuesFromRenderedElements` on `env.container`, then `formValuesToJSON(values, testSectionName)`), with the old step labels. The "then submit form" case keeps its `fireEvent.submit` on `getByRole("form")` with no assertion after it, as in the old file. No assertion changed.
- All 12 cases passed at their first GREEN run, with no React act warning. No case needed anything new in the environment (no matcher, portal, event behavior, or test-mode fork): `fireEvent.change` with a number or a bigint `value`, `fireEvent.click` on the checkbox, and `fireEvent.submit` behave as in the old run.
- Non-vacuity check, then reverted (the file was restored from a copy, `diff` empty): with `toHaveValue(101)` in "number allows to modify…" and `toHaveValue("98765432109876543211")` in "bigint allows to modify…", the SimpleType run without bail gave 2 failed and 10 passed, with `Expected <input name="TESTSECTION.testField"> to have value 101, received 100` and `… to have value "98765432109876543211", received "98765432109876543210"`.
- Old file: `getJzodSimpleTypeEditorTests`, `JzodSimpleTypes`, `LocalSimpleTypeEditorProps`, its two types (`JzodSimpleTypeEditorTest`, `JzodSimpleTypeEditorTestSuites`), and the `JzodSimpleTypeEditor` entry of `jzodElementEditorTests` are deleted and replaced by a comment that points to the new file. The now unused miroir-core imports `JzodPlainAttribute`, `JzodAttributePlainDateWithValidations`, `JzodAttributePlainNumberWithValidations`, and `JzodAttributePlainStringWithValidations` are removed. The other imports are still used by the Union, Any, and commented-out suites. Phase0: `JzodSimpleTypeEditor` is removed from the expected suite list, which is now `JzodUnionEditor`, `JzodAnyEditor`.

**Case list.** The new entry's `JzodSimpleTypeEditor` lines, reduced to `<suite> - jzodElementEditor - <case>: <status>`, are identical to the 12 `JzodSimpleTypeEditor` lines of `baseline-JzodElementEditor.txt` (`diff` empty). The old suite's reduced list (sorted) is identical to the 24 `JzodUnionEditor` and `JzodAnyEditor` lines of the baseline.

**Browser check (§4.4), development build only.** The Slice 8 script with `JzodSimpleTypeEditor` added to its result filter (`playwright-core` in the session scratchpad, headless Microsoft Edge, seed user `alice`, the MiroirTestDetails report of instance `761d4ed2-…`), against the Vite dev server already running on `https://localhost:5173` (not restarted), run twice. The panel showed after about 1 s and the run ended after about 14.6 s: "Passed: 44/44", "PASSED", 44 rows "All 1 assertions passed" (12 Array, 3 Enum, 3 Literal, 14 Object, 12 SimpleType). The sandbox kept the last case ("bigint allows to modify input value…", input `TESTSECTION.testField=98765432109876543210`), 0 console messages matched `act(`, and Close hid the panel and removed the container. The only console error was the known 403 on `/action/storeManagementAction_openStore` at page load. The Library navigation and the production build were not checked.

**Validation** (one command per file, from the repo root).

- `npx tsx packages/miroir-standalone-app/scripts/generate-component-miroir-tests.ts`: "5 suite(s), 44 case(s)", "no change".
- `npm run build -w miroir-test-app_deployment-miroir`: success.
- `npm run testByFile -w miroir-test-app_deployment-miroir -- modelValidation.unit.test.ts`: 153 passed.
- `npm run testByFile -w miroir-standalone-app -- componentMiroirTests.consistency`: 6 passed.
- `npm run testByFile -w miroir-standalone-app -- miroir-component-tests -t "JzodSimpleTypeEditor"`: 12 passed, 33 skipped.
- `npm run testByFile -w miroir-standalone-app -- miroir-component-tests`: 45 passed (44 cases and the entry check), no React act warning.
- `npm run testByFile -w miroir-standalone-app -- JzodElementEditor.test`: 24 passed.
- `npm run testByFile -w miroir-standalone-app -- componentMiroirTests.286.phase0`: 4 passed.
- `npm run testByFile -w miroir-standalone-app -- runAllComponentTests.286.phase6`: 3 passed (the manifest now gives 44 component leaves).
- `python scripts/check_bare_console.py`: OK.
- `npx tsc --noEmit --skipLibCheck -p packages/miroir-core/tsconfig.json`: 0 errors. `npx tsc --noEmit --skipLibCheck -p packages/miroir-standalone-app/tsconfig.json`: 1 error, the baseline `JzodElementEditorHooks.ts(528,59)` TS2339.

**Files created.** `packages/miroir-standalone-app/src/miroir-fwk/4-tests/componentTests/jzodElementEditor/JzodSimpleTypeEditor.tsx`.

**Files changed.** In `packages/miroir-standalone-app/src/miroir-fwk/4-tests/componentTests/`: `componentTestManifest.ts` and `componentTestRegistry.ts`. Tests: `tests/4_view/JzodElementEditor.test.tsx` and `componentMiroirTests.286.phase0.unit.test.tsx`. Deployment miroir: the instance `761d4ed2-….json`, written by the generator (44 leaves in 5 sub-suites). This plan.

**Note for Slice 12.** No new issue test in this slice. The tests to add to the nonreg unit beyond the §12.1 list stay as in the Slice 8 note (`componentTestFireEvent.286.phase8`).

#### Slice 10 realization (JzodUnionEditor)

**RED observed.** The 9 Union cases were added to `componentTestManifest.ts` (`JzodUnionEditor`, after `JzodSimpleTypeEditor`) and to `componentTestRegistry.ts` through a new `jzodElementEditor/JzodUnionEditor.tsx` whose 9 bodies threw "not migrated". The generator printed "6 suite(s), 53 case(s)" and wrote the instance once, then "no change" on a second run. `npm run build -w miroir-test-app_deployment-miroir` succeeded. The new entry, run with `npx vitest run` without bail (`VITE_TEST_MODE=true`): 9 failed, 45 passed (54). Each failure reads `reactComponentTest "JzodElementEditor_ComponentTestSuite#JzodUnionEditor#JzodUnionEditor: <case>" failed: not migrated`. The 44 Array, Enum, Literal, Object, and SimpleType cases and the entry check passed.

**GREEN.**

- `jzodElementEditor/JzodUnionEditor.tsx`: the 9 cases of `getJzodUnionEditorTests`, with the §5.3 rules (`screen` to `env.view`, `expect` to `env.expect`, `container` to `env.container`, `userEvent` to `env.userEvent`, React `act` to `env.act`, `waitFor` to `env.waitFor`, and `fireEvent` to `env.fireEvent`). `env.container` is the root of every `extractValuesFromRenderedElements` call and the target of the `querySelector` checks on the union type selector input. The one `console.log` became `env.log.info`, and the commented-out `screen.debug` lines were dropped.
- Three cases open a `ThemedSelectWithPortal` dropdown: the discriminator select, and the union type selector in the two "non-discriminated union can switch type…" cases. Their value reads pass `env.portalElement` as 7th argument, where the old wrapper searched `document.body`. The other value reads pass no portal element, as in Slices 8 and 9. In each dropdown case, the old single `act` (open, wait, clear, type, wait, Enter, wait) stays one `env.act` with the same steps in the same order.
- As in Slices 8 and 9, the old suite had no suite props, so each case keeps its full props. Three local constants hold the repeated parts: `testFieldProps` (label, name, and list keys), the two union schemas `stringOrNumberSchema` and `stringNumberOrObjectSchema`, and `unionTypeInputSelector` (`[data-testid="union-type-input-TESTSECTION.testField"]`). `suiteProps`, required by `ComponentTestSuite`, repeats the first case's props, and no case uses it. The unused `arrayValues` constant of the old factory was not ported. No assertion changed.
- All 9 cases passed at their first GREEN run, with no React act warning. No case needed anything new in the environment (no matcher, portal, event behavior, or test-mode fork). `toBeNull` and `.not.toBeNull` were already in the throwing `expect`. `env.waitFor` returns the callback's value as RTL's `waitFor` did, which the "places star and selector above the value" case uses. The user-event `clear`, `type`, and `keyboard("{Enter}")` sequence on the portaled selects behaves as in the old run.
- Non-vacuity check, then reverted (the file was restored from a copy, `diff` empty): with `type2Attribute: 1` in the discriminator case and `toBe(false)` for `starParent.contains(selectorInput)`, the Union run without bail gave 2 failed and 7 passed. The messages were `Expected {"type2Attribute":0,"testObjectType":"type2"} to equal {"testObjectType":"type2","type2Attribute":1}. First difference at path: ["type2Attribute"]` and `Expected true to be false`.
- Old file: `getJzodUnionEditorTests`, `LocalUnionEditorProps`, its two types (`JzodUnionEditorTest`, `JzodUnionEditorTestSuites`), and the `JzodUnionEditor` entry of `jzodElementEditorTests` are deleted and replaced by a comment that points to the new file. The now unused miroir-core import `JzodUnion` is removed. The other imports are still used by the Any suite or the commented-out suites. Phase0: `JzodUnionEditor` is removed from the expected suite list, which is now `JzodAnyEditor` alone.

**Case list.** The new entry's `JzodUnionEditor` lines, reduced to `<suite> - jzodElementEditor - <case>: <status>`, are identical to the 9 `JzodUnionEditor` lines of `baseline-JzodElementEditor.txt` (`diff` empty). The old suite's reduced list (sorted) is identical to the 15 `JzodAnyEditor` lines of the baseline.

**Browser check (§4.4), development build only.** Same method as Slice 9: the Slice 9 script with `JzodUnionEditor` added to its result filter, `playwright-core` in the session scratchpad, headless Microsoft Edge, seed user `alice`, and the MiroirTestDetails report of instance `761d4ed2-…`. It ran against the Vite dev server already running on `https://localhost:5173`, which was not restarted.

- The panel showed after about 0.9 s and the run ended after about 16.6 s, with "Passed: 53/53" and "PASSED".
- The sandbox kept the last case ("union type selector for object value places star and selector above the value"): the union type input `object {a, b}`, and the inputs `TESTSECTION.testField.a=hello` and `TESTSECTION.testField.b=1`. Its portal element was empty, and no option was outside the sandbox.
- 0 console messages matched `act(`. Close hid the panel and removed the container.
- The only console error was the known 403 on `/action/storeManagementAction_openStore` at page load.

The Library navigation and the production build were not checked.

**Validation** (one command per file, from the repo root).

- `npx tsx packages/miroir-standalone-app/scripts/generate-component-miroir-tests.ts`: "6 suite(s), 53 case(s)", "no change".
- `npm run build -w miroir-test-app_deployment-miroir`: success.
- `npm run testByFile -w miroir-test-app_deployment-miroir -- modelValidation.unit.test.ts`: 153 passed.
- `npm run testByFile -w miroir-standalone-app -- componentMiroirTests.consistency`: 6 passed.
- `npm run testByFile -w miroir-standalone-app -- miroir-component-tests -t "JzodUnionEditor"`: 9 passed, 45 skipped.
- `npm run testByFile -w miroir-standalone-app -- miroir-component-tests`: 54 passed (53 cases and the entry check), no React act warning.
- `npm run testByFile -w miroir-standalone-app -- JzodElementEditor.test`: 15 passed.
- `npm run testByFile -w miroir-standalone-app -- componentMiroirTests.286.phase0`: 4 passed.
- `npm run testByFile -w miroir-standalone-app -- runAllComponentTests.286.phase6`: 3 passed (the manifest now gives 53 component leaves).
- `python scripts/check_bare_console.py`: OK.
- `npx tsc --noEmit --skipLibCheck -p packages/miroir-core/tsconfig.json`: 0 errors. `npx tsc --noEmit --skipLibCheck -p packages/miroir-standalone-app/tsconfig.json`: 1 error, the baseline `JzodElementEditorHooks.ts(528,59)` TS2339.

**Files created.** `packages/miroir-standalone-app/src/miroir-fwk/4-tests/componentTests/jzodElementEditor/JzodUnionEditor.tsx`.

**Files changed.** In `packages/miroir-standalone-app/src/miroir-fwk/4-tests/componentTests/`: `componentTestManifest.ts` and `componentTestRegistry.ts`. Tests: `tests/4_view/JzodElementEditor.test.tsx` and `componentMiroirTests.286.phase0.unit.test.tsx`. Deployment miroir: the instance `761d4ed2-….json`, written by the generator (53 leaves in 6 sub-suites). This plan.

**Note for Slice 12.** No new issue test in this slice. The tests to add to the nonreg unit beyond the §12.1 list stay as in the Slice 8 note (`componentTestFireEvent.286.phase8`).

#### Slice 11 realization (JzodAnyEditor)

**RED observed.** The 15 Any cases were added to `componentTestManifest.ts` (`JzodAnyEditor`, after `JzodUnionEditor`) and to `componentTestRegistry.ts` through a new `jzodElementEditor/JzodAnyEditor.tsx` whose 15 bodies threw "not migrated" (the case props were already the old ones). The generator printed "7 suite(s), 68 case(s)" and wrote the instance once, then "no change" on a second run. `npm run build -w miroir-test-app_deployment-miroir` succeeded. The new entry, run with `npx vitest run` without bail (`VITE_TEST_MODE=true`): 15 failed, 54 passed (69). Each failure reads `reactComponentTest "JzodElementEditor_ComponentTestSuite#JzodAnyEditor#JzodAnyEditor: <case>" failed: not migrated`. The 53 Array, Enum, Literal, Object, SimpleType, and Union cases and the entry check passed.

**GREEN.**

- `jzodElementEditor/JzodAnyEditor.tsx`: the 15 cases of `getJzodAnyEditorTests`, with the §5.3 rules (`screen` to `env.view`, `expect` to `env.expect`, `container` to `env.container`, `userEvent` to `env.userEvent`, React `act` to `env.act`, `waitFor` to `env.waitFor`, `fireEvent` to `env.fireEvent`, and `waitAfterUserInteraction()` to the act-free `waitAfterUserInteraction(env.container)`). `env.container` is the root of every `extractValuesFromRenderedElements` call and the target of the `querySelector` checks on the union type selector input. The old suite had no `screen.debug` and no `log` call.
- Five cases open the `ThemedSelectWithPortal` union type selector ("any type can switch from number to string / number to object / object to string / object to array / array to string via selector"). Their value reads pass `env.portalElement` as 7th argument, as in Slice 10. The other value reads pass no portal element.
- As in Slices 8 to 10, the old suite had no suite props, so each case keeps its full props. The shared label, name, list keys, and `{ type: "any" }` schema are in a local `anyTestFieldProps`, and the selector in `unionTypeInputSelector`. `suiteProps`, required by `ComponentTestSuite`, repeats the first case's props, and no case uses it. The five switch cases repeated the same body with a different target type, so three local helpers hold it. `switchAnyType(env, targetType, expectedInitialType?)` runs the star click, the optional check of the current type (only in the three cases that had it: number, number, and array), and the old single `act` (open, wait, clear, type, wait, Enter, wait) as one `env.act` with the same steps in the same order. `waitForAnyValue(env, filter, label, step, expected)` is the old final `waitFor` (3000 ms) around the value read. `testFieldValues(env, step)` is the value read of the five button cases. Every step label, wait, and timeout is the old one. No assertion changed.
- All 15 cases passed at their first GREEN run, with no React act warning. No case needed anything new in the environment (no matcher, portal, event behavior, or test-mode fork).
- Non-vacuity check, then reverted (the file was restored from a copy, `diff` empty): with `a: "enter attributes here...X"` in "switch from number to object" and `toEqual(["item1", "item2"])` in "array can have an item removed", the Any run gave 2 failed and 13 passed, with `Expected {"a":"enter attributes here..."} to equal {"a":"enter attributes here...X"}. First difference at path: ["a"]` and `Expected ["item1","item3"] to equal ["item1","item2"]. First difference at path: ["1"]`.
- Old file: `getJzodAnyEditorTests` and the `JzodAnyEditor` entry of `jzodElementEditorTests` are deleted and replaced by comments that point to the new file. The now unused imports `act`, `fireEvent`, `waitFor` (`@testing-library/react`), `userEvent`, `getJzodElementEditorForTest`, and `waitAfterUserInteraction` are removed. `screen`, `extractValuesFromRenderedElements`, `formikFieldName`, `formValuesToJSON`, and `testSectionName` stay, because the Book, EntityDefinition, Performance, and Endpoint factories still use them. `jzodElementEditorTests` has only commented-out entries.
- **Placeholder in the old file.** With no active suite, `npm run testByFile -w miroir-standalone-app -- JzodElementEditor.test` failed with `Error: No test found in suite 4_view/JzodElementEditor.test.tsx` and `No test found in suite JzodElementEditor`. The file is still the `appstack-JzodElementEditor.test` nonreg default step until Slice 12, so the `describe("JzodElementEditor")` got one placeholder test, `has no active suite left, all moved to miroir-component-tests (#286)`, which asserts `Object.keys(jzodElementEditorTests)` equals `[]`. It goes away with the file in Slice 12.
- Phase0: the `pre-286 inventory` expected suite list is now empty (`toEqual([])`). That describe is deleted in Slice 12.

**Case list.** The new entry's full verbose list, reduced to `<suite> - jzodElementEditor - <case>: <status>` and sorted, has 68 lines and is identical to the sorted 68 lines of `baseline-JzodElementEditor.txt` (`diff` empty). The old suite now reports 1 test, the placeholder.

**Browser check (§4.4), development build only.** The Slice 10 script with `JzodAnyEditor` added to its result filter (`playwright-core` in the session scratchpad, headless Microsoft Edge, seed user `alice`, the MiroirTestDetails report of instance `761d4ed2-…`), against the Vite dev server already running on `https://localhost:5173` (not restarted).

- The panel showed after about 0.9 s and the run ended after about 20.7 s, with "Passed: 68/68" and "PASSED".
- The sandbox kept the last case ("any-typed array can have an attribute removed (it is a record)"): the inputs `TESTSECTION.testField.b-NAME=b` and `TESTSECTION.testField.b=world`. Its portal element was empty, and no option was outside the sandbox.
- 0 console messages matched `act(`. Close hid the panel and removed the container.
- The only console error was the known 403 on `/action/storeManagementAction_openStore` at page load.
...
- `npx tsx packages/miroir-standalone-app/scripts/generate-component-miroir-tests.ts`: "7 suite(s), 68 case(s)", "no change".
- `npm run build -w miroir-test-app_deployment-miroir`: success.
- `npm run testByFile -w miroir-test-app_deployment-miroir -- modelValidation.unit.test.ts`: 153 passed.
- `npm run testByFile -w miroir-standalone-app -- componentMiroirTests.consistency`: 6 passed.
- `npm run testByFile -w miroir-standalone-app -- miroir-component-tests -t "JzodAnyEditor"`: 15 passed, 54 skipped.
- `npm run testByFile -w miroir-standalone-app -- miroir-component-tests`: 69 passed (68 cases and the entry check), no React act warning.
- `npm run testByFile -w miroir-standalone-app -- JzodElementEditor.test`: 1 passed (the placeholder; 0 test before it was added, reported as a failure by vitest).
- `npm run testByFile -w miroir-standalone-app -- componentMiroirTests.286.phase0`: 4 passed.
- `npm run testByFile -w miroir-standalone-app -- runAllComponentTests.286.phase6`: 3 passed (the manifest now gives 68 component leaves).
- `python scripts/check_bare_console.py`: OK.
- `npx tsc --noEmit --skipLibCheck -p packages/miroir-core/tsconfig.json`: 0 errors. `npx tsc --noEmit --skipLibCheck -p packages/miroir-standalone-app/tsconfig.json`: 1 error, the baseline `JzodElementEditorHooks.ts(528,59)` TS2339.

**Files created.** `packages/miroir-standalone-app/src/miroir-fwk/4-tests/componentTests/jzodElementEditor/JzodAnyEditor.tsx`.

**Files changed.** In `packages/miroir-standalone-app/src/miroir-fwk/4-tests/componentTests/`: `componentTestManifest.ts` and `componentTestRegistry.ts`. Tests: `tests/4_view/JzodElementEditor.test.tsx` and `componentMiroirTests.286.phase0.unit.test.tsx`. Deployment miroir: the instance `761d4ed2-….json`, written by the generator (68 leaves in 7 sub-suites). This plan.

**Note for Slice 12.** No new issue test in this slice. The tests to add to the nonreg unit beyond the §12.1 list stay as in the Slice 8 note (`componentTestFireEvent.286.phase8`). Deleting `JzodElementEditor.test.tsx` also deletes its placeholder test.
