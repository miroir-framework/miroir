...
export class SqlDbModelStoreSection
  extends MixedSqlDbEntityAndInstanceStoreSection
  implements PersistenceStoreModelSectionInterface
{
  ...
  async getState(): Promise<{ [uuid: string]: EntityInstanceCollection }> {
    ...
    for (const parentUuid of this.getEntityUuids()) {
      ...
      const instances: Action2EntityInstanceCollectionOrFailure = await this.getInstances(
        parentUuid
      );
      ...
      if (instances instanceof Action2Error) {
        ...
        typeof instances.returnedDomainElement != "object" ||
        Array.isArray(instances.returnedDomainElement)
      ) {
        log.error(
          this.logHeader,
          "getState error getting instances for",
          parentUuid,
          "wrong type for instances:",
          instances
        );
        Object.assign(result, { [parentUuid]: { parentUuid, instances: [] } });
      } else {
