  ...
  return class MixedSqlDbEntityStore extends Base implements IAbstractEntityStore, IAbstractInstanceStore {
    ...
    async createEntity(entity: MetaEntity, entityDefinition: EntityDefinition):Promise<void> {
      if (entity.uuid != entityDefinition.entityUuid) {
        ...
        if (this.dataStore.getEntityUuids().includes(entity.uuid)) {
          log.warn(this.logHeader,'createEntity',entity.name,'already existing entity',entity.uuid,'existing entities',this.dataStore.getEntityUuids());
        } else {
    ...
    async dropEntity(entityUuid: string):Promise<void> {
      ...
      if(this.getEntityUuids().includes(entityEntityDefinition.uuid)) {
        ...
        log.warn(this.logHeader,'dropEntity sublevel for entityEntity does not exist',entityEntity.uuid,'existing entities',this.getEntityUuids());
      }
      ...
      if(this.getEntityUuids().includes(entityEntityDefinition.uuid)) {
        ...
        log.warn('StoreController dropEntity entity entityEntityDefinition does not exist',entityEntityDefinition.uuid,'existing entities',this.getEntityUuids());
      }
      return Promise.resolve();
    }
