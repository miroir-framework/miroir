...
/** Runners keyed by Runner `uuid` (leaf `runnerRef`) for `miroirTest_runner_library`. */
export const RUNNER_LIBRARY_RUNNER_REGISTRY: Record<string, Runner> = {
  [lendDocument.uuid]: lendDocument as unknown as Runner,
  [returnDocument.uuid]: returnDocument as unknown as Runner,
};
