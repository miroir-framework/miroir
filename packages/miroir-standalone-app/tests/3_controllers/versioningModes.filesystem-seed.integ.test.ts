/**
 * Filesystem emulated server seeds Version History from miroir_modelVersion assets.
 *
 * Run:
 * ```bash
...
const fileName = "versioningModes.filesystem-seed.integ.test";
...
describe("Miroir filesystem modelVersion seed", () => {
  ...
  it("bootstrapped store reads EntityVersion instances from modelVersion section", async () => {
    ...
    expect(entityVersions.length).toBe(
      MIROIR_VERSION_HISTORY_PARENTS[entityEntityVersion.uuid!],
    );
  ...
  it("same EntityVersion UUIDs are not returned from data section", async () => {
    ...
    // FAILS BY DESIGN — documents https://github.com/miroir-framework/miroir/issues/239:
    // boot currently writes 17 EntityVersion + 1 SelfApplicationVersion instances into the data section.
    // Keep failing-visible; do not patch src/ or weaken this assertion to make it pass.
  ...
  it("SelfApplicationVersion seed rows are readable from modelVersion only", async () => {
    ...
    expect(modelVersionRows.length).toBe(MIROIR_VERSION_HISTORY_PARENTS[savParentUuid]);
