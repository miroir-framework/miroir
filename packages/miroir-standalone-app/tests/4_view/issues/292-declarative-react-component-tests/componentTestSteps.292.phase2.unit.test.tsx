...
describe("componentTestSteps.292.phase2: Enum fixture on the real JzodElementEditor", () => {
  ...
  it("a kind not implemented yet gives step 1 (clickArrayButton): not implemented", async () => {
    const result: any = await newRunner()({
      testNamePath: [...enumSuite.suitePath, enumCaseLabel],
      leaf: stepLeaf([{ step: "clickArrayButton", field: "testField", action: "add" }]),
      suite: enumSuite,
    });
    expect(result).toEqual({ status: "error", message: "step 1 (clickArrayButton): not implemented" });
  });
