/**
 * #252 Slice 7 — final UI_INTEGRATION_RUNNER_SUITE_REGISTRY shape (no playfield fallback).
...
const shouldRun =
  !RUN_TEST ||
  RUN_TEST === "runnerRegistry.252.phase7" ||
  RUN_TEST === "runnerRegistry.252.phase7.unit.test";
...
(shouldRun ? describe : describe.skip)(
  "UI integration runner registry final contract (issue #252 slice 7)",
  () => {
    ...
    it("every entry is kind + suiteDefinition and optional init; never a registry playfield", () => {
      for (const key of EXPECTED_KEYS) {
        ...
        expect(Object.keys(entry).sort(), key).toEqual(
          Object.keys(entry)
            .filter((field) => ALLOWED_ENTRY_KEYS.has(field))
            .sort(),
        );
