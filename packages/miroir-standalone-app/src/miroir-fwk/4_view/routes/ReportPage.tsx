...
export const ReportPage = (props: ReportPageProps) => {
  ...
  const availableReports: Report[] = displayedDeploymentDefinition?.applicationModelLevel == "metamodel"?(
    deploymentReports.filter(r=>(
        ([reportEntityList.uuid,reportEntityDefinitionList.uuid].includes(r.uuid) && params.applicationSection == 'model') 
        ||
        (!([reportEntityList.uuid,reportEntityDefinitionList.uuid].includes(r.uuid)) && params.applicationSection == 'data')
      )
    )
  ):deploymentReports;
