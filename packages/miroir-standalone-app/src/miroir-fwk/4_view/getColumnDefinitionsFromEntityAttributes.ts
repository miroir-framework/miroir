import { ColDef } from "ag-grid-community";


import {
  EntityVersion,
  JzodElement,
  LoggerInterface,
  MiroirLoggerFactory,
  type ObjectListReportSection,
  type ReportSection
} from "miroir-core";

import { packageName } from "../../constants.js";
import { EntityInstanceCellRenderer } from "./components/Grids/EntityInstanceCellRenderer.js";
import { TableComponentRow } from "./components/Grids/EntityInstanceGridInterface.js";
import { GenderCellEditor } from "./components/Grids/GenderCellEditor.js";
import GenderCellRenderer from "./components/Grids/GenderCellRenderer.js";
import type { AutoGeneratedColumnDef } from "./components/Grids/ValueObjectGridInterface.js";
import {
  DefaultCellRenderer,
} from "./components/SelectEntityInstanceEditor.js";
import { cleanLevel } from "./constants.js";

let log: LoggerInterface = console as any as LoggerInterface;
MiroirLoggerFactory.registerLoggerToStart(
  MiroirLoggerFactory.getLoggerName(packageName, cleanLevel, "getColumnDefinitionsFromEntityDefinitionAttribute"), "UI",
).then((logger: LoggerInterface) => {log = logger});


// ################################################################################################
export function getMDataGridColumnDefinition(
  deploymentUuid: string, // prop drilling
  name: string,
  mlSchema: JzodElement,
  reportSectionDefinition: ObjectListReportSection | undefined,
  entityVersion?:
    | EntityVersion
    | { viewAttributes?: string[] | undefined; uuid?: string; entityUuid?: string; name?: string }
    | undefined,
): AutoGeneratedColumnDef {
  const sortParams =
    reportSectionDefinition?.definition.sortByAttribute === name
      ? { sort: reportSectionDefinition.definition.sortOrder ?? "asc", sortIndex: 0 }
      : {};

  if (mlSchema?.tag?.value?.foreignKeyParams?.targetEntity) {
    const result: AutoGeneratedColumnDef =  {
      field: name,
      ...sortParams,
      headerName: mlSchema.tag?.value?.defaultLabel ? mlSchema.tag?.value?.defaultLabel : name,
      cellRenderer: EntityInstanceCellRenderer,
      // cellEditor: SelectEntityInstanceEditorNotUsed, // can not be edited in table column: click navigates to instance details report page
      // cellEditorPopup: true,
      // editable: true,
      // sort:'asc',
      // cellEditorParams: {
      //   entityUuid: mlSchema?.tag?.value?.targetEntity,
      // },
      cellRendererParams: {
        deploymentUuid,
        isFK: true,
        entityUuid: mlSchema?.tag?.value?.foreignKeyParams?.targetEntity,
        entityVersion
      },
      // width: defaultWidth,
    };
    // log.info(
    //   "column with targetEntity named",
    //   name,
    //   "mlSchema",
    //   mlSchema,
    //   "targetEntity",
    //   mlSchema?.tag?.value?.targetEntity,
    //   "entityPublisher.uuid",
    //   entityPublisher.uuid,
    //   entityPublisher.uuid == mlSchema?.tag?.value?.targetEntity,
    //   "result",
    //   result
    // );

    return result;
  }
  // log.info(
  //   "getColumnDefinitionsFromEntityDefinitionAttribute name column",
  //   name,
  //   "mlSchema",
  //   mlSchema,
  //   // "jzodObjectSchema",
  //   // jzodObjectSchema,
  //   "entityVersion",
  //   entityVersion
  // );


  switch (name) {
    case "uuid":
    case "name": {

      return {
        field: name,
        headerName: mlSchema.tag?.value?.defaultLabel ? mlSchema.tag?.value?.defaultLabel : name,
        ...sortParams,
        cellRenderer: EntityInstanceCellRenderer,
        // cellEditor: SelectEntityInstanceEditorNotUsed,
        // cellEditorPopup: true,
        // editable: true,
        // cellEditorParams: {
        //   entityUuid: entityVersion?.uuid??"",
        // },
        cellRendererParams: {
          deploymentUuid,
          entityUuid: entityVersion?.entityUuid??"",
          entityVersion
        },
        // width: defaultWidth,
      };
      break;
    }
    case "gender": { // TODO: not used anymore, come up with another similar example 
      return {
        field: "gender",
        ...sortParams,
        headerName: mlSchema.tag?.value?.defaultLabel ? mlSchema.tag?.value?.defaultLabel : name,
        cellRenderer: GenderCellRenderer,
        cellEditor: GenderCellEditor,
        cellEditorPopup: true,
        editable: true,
        // width: defaultWidth,
        cellRendererParams: {
          deploymentUuid,
          entityUuid: entityVersion?.entityUuid??"",
          entityVersion
        },
      };
      break;
    }
    case "conceptLevel": { // TODO: not used anymore, come up with a general solution for enums
      // log.info("column conceptLevel", name, mlSchema);
      return {
        field: name,
        ...sortParams,
        headerName: mlSchema.tag?.value?.defaultLabel ? mlSchema.tag?.value?.defaultLabel : name,
        // width: defaultWidth,
        cellRendererParams: {
          deploymentUuid,
          entityUuid: entityVersion?.entityUuid??"",
          entityVersion
        },
      };
    }
    default: {
      if (!mlSchema) {
        log.error(
          "getColumnDefinitionsFromEntityDefinitionAttribute: mlSchema is undefined for name",
          name,
          "entityVersion",
          entityVersion
        );
      } else {
        // log.info("column default:", name, mlSchema);
      }
      return {
        field: name,
        ...sortParams,
        headerName: mlSchema.tag?.value?.defaultLabel
          ? mlSchema.tag?.value?.defaultLabel
          : name,
        cellRenderer: DefaultCellRenderer,
        cellRendererParams: {
          columnName: name,
          deploymentUuid,
          entityUuid: entityVersion?.entityUuid ?? "",
          entityVersion,
        },
        // cellRendererParams: {
        //   deploymentUuid,
        //   columnName: name,
        // },
      };
      break;
    }
  }
}

// ################################################################################################
export function getMDataGridColumnDefinitionsFromEntityDefinition(
  deploymentUuid: string,
  mlSchema: JzodElement | undefined,
  reportSectionDefinition: ObjectListReportSection | undefined,
  // reportSectionDefinition: ReportSection | undefined,
  entityVersion?:
    | EntityVersion
    | { viewAttributes?: string[] | undefined; uuid?: string; entityUuid?: string; name?: string }
    | undefined,
  rowData?: TableComponentRow[],
  availableWidth?: number
): ColDef<any>[] {
  log.info(
    "getMDataGridColumnDefinitionsFromEntityDefinition",
    "deploymentUuid",
    deploymentUuid,
    "mlSchema",
    mlSchema,
    "viewAttributes",
    entityVersion?.viewAttributes,
    "entityVersion",
    entityVersion,
    "reportSectionDefinition",
    reportSectionDefinition
  );
  
  // let columnDefs: ColDef<any>[] = [];
  let columnDefs: AutoGeneratedColumnDef[] = [];
  
  switch (mlSchema?.type) {
    case "object":
      {
        const schemaKeys = Object.keys(mlSchema.definition)
        if (entityVersion?.viewAttributes) {
          columnDefs = entityVersion.viewAttributes
            .filter((a: string) => [a, schemaKeys.find((b) => b == a)])
            .map((a: string) =>
              getMDataGridColumnDefinition(
                deploymentUuid,
                a,
                mlSchema.definition[a],
                reportSectionDefinition,
                entityVersion,
              )
            );
        } else {
          columnDefs = Object.entries(mlSchema.definition ? mlSchema.definition : {})
            .map((e: [string, any]) =>
              getMDataGridColumnDefinition(deploymentUuid, e[0], e[1], reportSectionDefinition, entityVersion)
            );
        }
      }
      break;
    default: {
      columnDefs = [];
      break;
    }
  }
  // // Calculate adaptive widths if row data is provided
  // if (rowData && rowData.length > 0 && columnDefs.length > 0) {
  //   const widthSpecs = calculateAdaptiveColumnWidths(
  //     columnDefs,
  //     rowData,
  //     availableWidth || 1200,
  //     {
  //       // field: "tools",
  //       field: "", // TODO: THIS IS WRONG!!!!!!!
  //       headerName: "Actions",
  //       width: 120,
  //     },
  //     mlSchema?.type === "object" ? mlSchema.definition : undefined
  //   );

  //   // // Apply calculated widths to column definitions
  //   // columnDefs.forEach((colDef, index) => {
  //   //   const widthSpec = widthSpecs.find(spec => spec.field === colDef.field);
  //   //   if (widthSpec) {
  //   //     colDef.width = Math.round(widthSpec.calculatedWidth);
  //   //     colDef.minWidth = Math.round(widthSpec.minWidth);
  //   //     colDef.maxWidth = Math.round(widthSpec.maxWidth);
  //   //     // colDef.suppressSizeToFit = true;// defined by Ag-Grid to limit auto-sizing
  //   //     // colDef.suppressAutoSize = true;
  //   //   }
  //   // });
  // }

  return columnDefs;
}
