...
import { MlElement, MlObject, LoggerInterface, MiroirLoggerFactory } from "miroir-core";
...
// ################################################################################################
const _miroirLoggerName = MiroirLoggerFactory.getLoggerName(packageName, cleanLevel, "ValueObjectGrid");
...
const generateColumnDefsFromSchema = (mlSchema: MlObject): AutoGeneratedColumnDef[] => {
  ...
  if (mlSchema.definition) {
    Object.entries(mlSchema.definition).forEach(([fieldName, fieldSchema]) => {
      const mlElement = fieldSchema as MlElement;
      ...
      if (mlElement.type === "number") {
        columnType = "number";
      } else if (mlElement.type === "boolean") {
        columnType = "boolean";
      } else if (mlElement.type === "date") {
        columnType = "date";
      } else if (mlElement.type === "object") {
        columnType = "object";
        // For objects, we'll stringify them for display
        cellRenderer = "jsonCellRenderer";
      } else if (mlElement.type === "array") {
        columnType = "array";
        cellRenderer = "jsonCellRenderer";
      }
        ...
        .map((valueObject: any, index: number) => ({
          ...
          displayedValue: Object.fromEntries(
            Object.entries(valueObject).map(([key, value]) => {
              const fieldSchema = props.mlSchema.definition?.[key] as MlElement;
              return [
                key,
