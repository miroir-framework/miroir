...
export const EntityInstanceGrid = (props: TableComponentProps) => {
  ...
  const rowOpenReport =
    props.type === TableComponentTypeSchema.enum.EntityInstance ? props.rowOpenReport : undefined;
  const onRowOpenReport =
    props.type === TableComponentTypeSchema.enum.EntityInstance ? props.onRowOpenReport : undefined;

  const handleRowOpenReport = useCallback((row?: TableComponentRow) => {
    if (!row || !onRowOpenReport) {
      return;
    }
    const instanceUuid = getInstancePrimaryKeyValue(presentModelSource ?? {}, row.rawValue);
    onRowOpenReport(row, instanceUuid);
  }, [onRowOpenReport, presentModelSource]);
    ...
    const baseColumnDefs: AutoGeneratedColumnDef[] = [
      {
        ...
        cellRendererParams: {
          ...
          onClickOpen: rowOpenReport ? handleRowOpenReport : undefined,
        },
