...
export interface UnitTestExecutionSummaryProps {
  testResultsData: any[];
  testLabel?: string;
}

export const UnitTestExecutionSummary: React.FC<UnitTestExecutionSummaryProps> = ({
  testResultsData,
  testLabel = "UnitTest",
}) => {
  ...
  testResultsData.forEach((testData: any) => {
    ...
    if (hasSkippedAssertion && !hasFailedAssertion && !hasPassedAssertion) {
      ...
      hasFailedAssertion ||
      testData.testResult === "error" ||
      testData.status === "error" ||
      (testData.failedAssertions && testData.failedAssertions.length > 0)
    ) {
      failedTests++;
  ...
  return (
    <div style={{
      ...
      <h4 style={{
        ...
        {testLabel.toUpperCase()} EXECUTION SUMMARY
      </h4>
