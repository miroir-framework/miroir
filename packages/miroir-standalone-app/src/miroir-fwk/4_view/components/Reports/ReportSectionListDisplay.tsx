...
import { DEFAULT_GRID_PAGE_SIZE } from "../Grids/gridPagination.js";
...
const DEFAULT_LIST_GRID_SIZING: ListGridSizingSnapshot = {
...
export const ReportSectionListDisplay: React.FC<ReportComponentProps> = (
  ...
  const restoredGridSizing = DEFAULT_LIST_GRID_SIZING;
  const [displayedPageInstances, setDisplayedPageInstances] =
    useState<EntityInstancesUuidIndex>({});
  ...
  const effectiveGridSizing = useMemo(
    () =>
      transformerPanelEnabled
        ? { pageSize: LIST_TRANSFORMER_PAGE_SIZE }
        : restoredGridSizing,
    [restoredGridSizing, transformerPanelEnabled],
  );

  const handleTransformerToggle = useCallback(() => {
    setTransformerPanelEnabled((enabled) => {
      if (enabled) {
        setDisplayedPageInstances({});
        return false;
      }
  ...
  const handleDisplayedPageRowsChange = useCallback(
    (pageInstances: EntityInstancesUuidIndex) => {
      setDisplayedPageInstances((previous) => {
        const previousKeys = Object.keys(previous).sort().join(",");
        const nextKeys = Object.keys(pageInstances).sort().join(",");
  ...
  const instancesForTransformerPanel = useMemo(() => {
    if (!transformerPanelEnabled) {
      return {};
    }
    if (Object.keys(displayedPageInstances).length > 0) {
      return displayedPageInstances;
    }
    return sliceInstancesToPage(
      instancesToDisplay,
      0,
      LIST_TRANSFORMER_PAGE_SIZE,
      objectListReportSection?.definition?.sortByAttribute,
            ...
            {props.deploymentUuid ? (
              <div>
                <EntityInstanceGrid
                  ...
                  deploymentUuid={props.deploymentUuid}
                  displayTools={true}
                  {...(effectiveGridSizing.pageSize != null
                    ? { pageSize: effectiveGridSizing.pageSize }
                    : effectiveGridSizing.maxRows != null
                      ? { maxRows: effectiveGridSizing.maxRows }
                      : { pageSize: DEFAULT_GRID_PAGE_SIZE })}
                  onDisplayedPageRowsChange={
                    transformerPanelEnabled ? handleDisplayedPageRowsChange : undefined
                  }
