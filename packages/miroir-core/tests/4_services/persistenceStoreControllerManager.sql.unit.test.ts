/**
 * Postgres startup: a versioned SQL deployment opens a distinct modelVersion
 * schema, isolated from the live model schema.
 *
 * Gated on Postgres availability: the suite skips when no test database can be
 * reached (MIROIR_TEST_POSTGRES_HOST, default localhost).
 */
...
function buildSqlConfig(runId: string): StoreUnitConfiguration {
  return {
    admin: {
      ...
      connectionString,
      schema: `modelversion_admin_${runId}`,
      forceNullOptionalAttributeToUndefined: true,
    },
    model: {
      emulatedServerType: "sql",
      connectionString,
      schema: `modelversion_library_${runId}`,
      forceNullOptionalAttributeToUndefined: true,
    },
    data: {
      emulatedServerType: "sql",
    ...
    modelVersion: {
      emulatedServerType: "sql",
      connectionString,
      schema: `modelversion_library_mv_${runId}`,
      forceNullOptionalAttributeToUndefined: true,
    },
  };
}

describe("PersistenceStoreControllerManager SQL modelVersion startup", () => {
  beforeAll(async () => {
    try {
  ...
  it.skipIf(!postgresAvailable)(
    "versioned SQL deployment opens distinct modelVersion schema and isolates history rows",
    async () => {
      ...
      expect(historyStoreName).toContain(`modelversion_library_${runId}`);
