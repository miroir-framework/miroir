/**
 * PersistenceStoreControllerManager startup and backend support policy for the
 * modelVersion (history) section.
 *
 * Startup: a versioned deployment registers and opens a distinct modelVersion
 * storage next to model and data; an unversioned deployment keeps working
 * without it and reports a named error on history access.
 *
 * Backend policy: indexedDb / mongodb backends accept a distinct modelVersion
 * storage; the bundled (read-only) backend rejects history persistence with an
 * explicit error and stays valid without a modelVersion section.
 */
...
class RecordingSectionStore implements PersistenceStoreDataSectionInterface {
  ...
  constructor(
    ...
    public readonly historyInstance?: { uuid: string; parentUuid: string },
  ) {}
  ...
  async getInstances() {
    return {
      returnedDomainElement: {
        ...
        elementValue: {
          parentUuid: this.historyInstance?.parentUuid ?? ENTITY_UUID,
          applicationSection: this.section,
          instances:
            this.section === "modelVersion" && this.historyInstance ? [this.historyInstance] : [],
        },
      },
    } as any;
  }
...
describe("PersistenceStoreControllerManager startup with modelVersion storage", () => {
  let tempRoot: string;
  ...
  it("versioned deployment registers distinct modelVersion storage and opens it", async () => {
    tempRoot = await mkdtemp(join(tmpdir(), "modelVersion-startup-"));
    const modelStore = new RecordingModelStore("model", "model-store");
    const dataStore = new RecordingSectionStore("data", "data-store");
    const historyStore = new RecordingSectionStore("modelVersion", "history-store", INSTANCE);
  ...
  it("unversioned deployment without modelVersion still opens model and data", async () => {
    tempRoot = await mkdtemp(join(tmpdir(), "unversioned-startup-"));
    const modelStore = new RecordingModelStore("model", "model-store");
    const dataStore = new RecordingSectionStore("data", "data-store");
...
describe("modelVersion backend support policy", () => {
  let tempRoot: string;
  ...
  it.each(["indexedDb", "mongodb"] as const)(
    "%s registers distinct modelVersion storage and routes history reads there",
    async (storageType) => {
      tempRoot = await mkdtemp(join(tmpdir(), `policy-${storageType}-`));
      ...
      const historyStore = new RecordingSectionStore(
        "modelVersion",
        `${storageType}-history`,
        HISTORY_INSTANCE,
      );
  ...
  it("bundled modelVersion rejects history persistence with an explicit read-only error", async () => {
    tempRoot = await mkdtemp(join(tmpdir(), "bundled-policy-"));
    ...
    const writeResult = await controller.upsertInstance("modelVersion", HISTORY_INSTANCE);
  ...
  it("bundled deployments without modelVersion remain valid (read-only live model only)", async () => {
    tempRoot = await mkdtemp(join(tmpdir(), "bundled-unversioned-"));
    miroirBundledStoreSectionStartup(ConfigurationService.configurationService, {
      [DEPLOYMENT_UUID]: { admin: {}, model: {}, data: {} },
    });
