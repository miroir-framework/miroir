...
import { metaMetaModelEntityUuids, metaModelEntityUuids } from "./Model";
...
export const getApplicationSection = (
  deploymentUUid: string,
  entityUuid: string,
  applicationSection?: ApplicationSection,
): ApplicationSection => {
  if (deploymentUUid == adminConfigurationDeploymentMiroir.uuid) {
    // return entityUuid == entityEntity.uuid || entityUuid == entityEntityDefinition.uuid
    return metaMetaModelEntityUuids.includes(entityUuid)
      ? "model"
      : "data";
  } else {
    return metaModelEntityUuids.includes(entityUuid)
