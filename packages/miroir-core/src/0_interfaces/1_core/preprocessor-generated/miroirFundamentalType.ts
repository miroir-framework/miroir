...
export const queryExtractorTransformerAbstract: z.ZodType<QueryExtractorTransformerAbstract> = z.object({queryType:z.literal("extractorTransformer"), referencedExtractor:z.lazy(() =>queryTemplateContextReference)}).strict();
export const queryExtractorTransformerUnique: z.ZodType<QueryExtractorTransformerUnique> = z.object({queryType:z.literal("extractorTransformer"), referencedExtractor:z.lazy(() =>queryTemplateContextReference)}).strict().extend({queryName:z.literal("unique"), attribute:z.string()}).strict();
export const queryExtractorTransformerCount: z.ZodType<QueryExtractorTransformerCount> = z.object({queryType:z.literal("extractorTransformer"), referencedExtractor:z.lazy(() =>queryTemplateContextReference)}).strict().extend({queryName:z.literal("count"), groupBy:z.string().optional()}).strict();
...
export const queryAction: z.ZodType<QueryAction> = z.object({actionType:z.literal("queryAction"), actionName:z.literal("runQuery"), endpoint:z.literal("9e404b3c-368c-40cb-be8b-e3c28550c25e"), deploymentUuid:z.string().uuid(), query:z.union([z.lazy(() =>domainModelSingleExtractor), z.lazy(() =>extractorForRecordOfExtractors)])}).strict();
