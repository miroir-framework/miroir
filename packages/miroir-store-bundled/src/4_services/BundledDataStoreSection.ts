...
export class BundledDataStoreSection
  extends BundledStore
  implements PersistenceStoreDataSectionInterface
{
  ...
  constructor(
    ...
    super(storeName, "BundledDataStoreSection " + storeName);
  ...
  async getInstance(
    ...
    if (!instance) {
      return Promise.resolve(
        new Action2Error(
          "FailedToGetInstance",
          `getInstance: instance ${instancePrimaryKey} of entity ${parentUuid} not found in bundled store`,
        ),
      );
    }
    return Promise.resolve({
