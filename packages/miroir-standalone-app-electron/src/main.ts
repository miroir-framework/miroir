...
class MainWindow {
  ...
  private async loadApp(): Promise<void> {
    ...
    if (this.isDev) {
      ...
      // Uses HTTPS when certs are available (generated by scripts/setup-https.sh/.ps1),
      // falls back to HTTP when certs are absent (graceful degradation).
      this.mainWindow.loadURL(`${_devServerScheme}://localhost:5173/home`);
    } else {
