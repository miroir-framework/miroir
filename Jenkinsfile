pipeline {
    ...
    stages {
        ...
        stage('test') { 
            steps {
              sh 'cd /home/tmp/miroir-standalone-app-ci && npm run test -w miroir-standalone-app --env=./tests/miroirConfig.test-ci-emulatedServer-sql.json -- DomainController.Model.CRUD'
