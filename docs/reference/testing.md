...
**Persistent named secrets (#270):** PLATFORM vitest under `tests/**/issues/270-persistent-named-secrets/` (`secrets.270`, `secretsService.270`, `secretsHttp.270`, `secretsRedact.270`, `secretsHydrate.270`, `secretsPrincipal.270`, `secretsRotation.270`, `secretsImport.270`, `secretsPage.270`). Nonreg steps `unit-270-persistent-secrets` (core + MCP) and `appstack-270-persistent-secrets` (standalone-app, pinned to `emulatedServer-filesystem`). Production launch uses wrapping key `MIROIR_SECRETS_MASTER_KEY` / `--secrets-master-key`; `--secret` / `MIROIR_SECRET_*` / AI key env vars are bootstrap import only.
