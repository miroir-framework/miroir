...
## Documented inconsistencies with existing docs

These are gaps between older guides / API sketches and the **current** model assets & issues. Prefer this guide + live `*_model` JSON + `EntityPrimaryKey.ts` when they disagree.

| # | Where | What is wrong / outdated | Current reality |
|---|--------|---------------------------|-----------------|
| 1 | [Core Concepts](../core-concepts.md), [entity.md](../../reference/api/entity.md) | Field named **`jzodSchema`** on EntityDefinition | Live definitions and the Library tutorial use **`mlSchema`** (Jzod object). EntityDefinition meta-schema also names the attribute `mlSchema`. |
| 2 | [entity.md](../../reference/api/entity.md) `conceptLevel` | Enum shown as `MetaModel \| Model \| Data` only | Schema includes **`External`** (required for Postgres catalogue Entities). |
| 3 | [entity.md](../../reference/api/entity.md) Entity.`parentUuid` | Claims instances of Entity always parent to `381ab1be-…` | `381ab1be-…` is the **EntityDefinition** of Entity. Instance `parentUuid` for Entities (e.g. Book) is the Entity uuid **`16dbfe28-…`**. EntityDefinition instances parent to EntityDefinition Entity **`54b9c72f-…`**. |
| 4 | [Library tutorial](../../tutorials/library-tutorial.md) | “Every entity instance **must** have” `uuid` and `parentUuid` | Default path still does; [#172](https://github.com/miroir-framework/miroir/issues/172) / [#173](https://github.com/miroir-framework/miroir/issues/173) allow optional `parentUuid` and non-UUID / composite PKs. Tutorial `conceptLevel` text also omits `MetaModel` / `External`. |
| 5 | [Core Concepts](../core-concepts.md) Book EntityDefinition sample | Invented definition uuid / uses `jzodSchema` / no `mlSchema` extend root | Prefer Library asset `797dd185-…` (extends `entityDefinitionRoot`). |
| 6 | [creating-applications.md](creating-applications.md), [integration.md](integration.md) | Still placeholders for “define entities” / “external databases” | This guide + Postgres External Entities are the working narrative until those pages are filled. |
| 7 | [entity.md](../../reference/api/entity.md) status | Marked sketch / “SLOPPY”; still useful for property lists | Treat as non-authoritative next to generated types and `bdd7ad43-…` EntityDefinition schema. |
| 8 | Issue [#175](https://github.com/miroir-framework/miroir/issues/175) vs [#173](https://github.com/miroir-framework/miroir/issues/173) | Easy to confuse “no `idAttribute`” with “no PK” | **Absent `idAttribute` ⇒ default `uuid` PK.** PK-less needs an explicit representation beyond omitting `idAttribute`. |

Issues [#172](https://github.com/miroir-framework/miroir/issues/172)–[#176](https://github.com/miroir-framework/miroir/issues/176) remain **OPEN** on GitHub even where substantial implementation and tests already exist; treat issue state as product tracking, not “unimplemented.”
