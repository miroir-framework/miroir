...
| **inputReportSection** | Yes — TVOE with no submit button | Optional `onChangeVector` (e.g. `application` → `navigate` to same report under another app). When the section definition declares `urlParamFields: string[]` (#267), an **OK** button is rendered: clicking it writes the named input values into the report URL search params (navigate), so the report query re-runs with the new pageParams; those fields are also seeded from the current pageParams. Reports with a `urlParamFields` input keep rendering their sections (plus an error banner) when the query fails, so the input stays reachable |
